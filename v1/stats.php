		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.match(error)) {
					...
					var statsChart = new Chart(ctx).Line(chartData,{responsive: true, maintainAspectRatio: false, legendTemplate : "<ul class=\"<%=name.toLowerCase()%>-legend pull-left list-group\"><% for (var i=0; i<datasets.length; i++){%><li class=\"list-group-item\"><div style=\"float:left !important; padding:5px !important; height:15px !important;width:15px !important;background-color:<%=datasets[i].pointColor%>\"></div><%if(datasets[i].label){%><%=datasets[i].label%><%}%></li><%}%></ul>"});
