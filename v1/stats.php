		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.match(error)) {
					...
					var statsChart = new Chart(ctx).Line(chartData,{responsive: true, scaleBeginAtZero: true, legendTemplate : "<ul class=\"<%=name.toLowerCase()%>-legend pull-left\"><% for (var i=0; i<datasets.length; i++){%><li><p  style=\"height:5px !important;width:5px !important;background-color:<%=datasets[i].fillColor%>\"></p><%if(datasets[i].label){%><%=datasets[i].label%><%}%></li><%}%></ul>"});
