...
if(session_id() == '') {
	...
	if(isset($_POST['populateStatsChart']) && !empty($_POST['populateStatsChart'])) {
		...
		$selectStats = "SELECT ROUNT(SUM(amtateweight),2) AS amtateweight, ROUND(AVG(petweight),2) AS petweight, event_time FROM $GLOBALS[schema].stats WHERE tag_id = $1 AND feeder_id = $2 GROUP BY event_time ORDER BY event_time ASC";
